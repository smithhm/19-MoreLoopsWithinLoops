...
def run_test_largest_number():
    ...
    # Test 4:
    expected = 100
    answer = largest_number(([-1, 4, 7, -11, 19], [100], [-100, 20, 9]))
    print('Expected and actual are:', expected, answer)
...
def largest_number(seq_seq):
    ...
    if len(seq_seq) == 0:
        return None
    if len(seq_seq[0]) != 0:
        greatest = seq_seq[0][0]
    elif len(seq_seq[1]) != 0:
        greatest = seq_seq[1][0]
    else:
        return None
    for k in range(len(seq_seq)):
        for j in range(len(seq_seq[k])):
...
def run_test_largest_negative_number():
    ...
    # Test 1
    expected = -1.9
    answer = largest_negative_number(([15, -1.9, 11], [-4, -8, -2, -14], [0, -33, 55, -2.5]))
    print('Expected and actual are:', expected, answer)

    # Test 2
    expected = None
    answer = largest_negative_number(([], [4, 56, 9, 0], [1]))
    print('Expected and actual are:', expected, answer)

    # Test 3
    expected = -19999999
    answer = largest_negative_number(([3, 80, 17, -1000000001, -19999999, 0], [9, 8, 7, 6.6,], []))
    print('Expected and actual are:', expected, answer)
...
def largest_negative_number(seq_seq):
    ...
    # -------------------------------------------------------------------------
    larg_neg = -1000000000000000000000
    for k in range(len(seq_seq)):
        for j in range(len(seq_seq[k])):
            if seq_seq[k][j] < 0:
                if seq_seq[k][j] > larg_neg:
                    larg_neg = seq_seq[k][j]
    if larg_neg == -1000000000000000000000:
        return None
    else:
        return larg_neg
